...
beta_safe=${beta// /_}
job_name="n${n}_B${B}_beta${beta_safe}_alpha${alpha}_iter${iterations}_strue${strue}_snull${snull}"
...
python main_submit.py --n "$n" --B "$B" --beta "$beta" --alpha "$alpha" --iterations "$iterations" --strue "$strue" --snull "$snull" > "$output_file"
