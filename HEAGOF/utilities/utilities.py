...
def generate_s(n,beta,snull,loc=0.5,strength=0.5,width=5):
    """
    Generate expected counts s under the null H0 without RMF, ARE, ...
...
def generate_s_true(n,beta,strue,snull,loc=0.5,strength=10,width=5):
    """
    Generate expected counts s under the true model without RMF, ARE, ...
...
def numerical_jacobian(func, theta, eps=1e-6):
    theta = np.array(theta, dtype=float)
    v0 = func(theta)                 # v0.shape = (n,)
    n = len(v0)
    d = len(theta)
    J = np.zeros((n, d))
    ...
    for j in range(d):
        ...
        theta_eps[j] += eps
        v1 = func(theta_eps)
        J[:, j] = (v1 - v0) / eps     # differentiate
    return J

def powerlaw_func(x,theta):
