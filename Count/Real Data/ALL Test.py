...
ARE=np.array([1 for i in range(blockLength)])
RMF=np.eye(blockLength)
BACK=0
left=0
right=0
Energy=np.array([1.0+i/blockLength for i in range(blockLength+1)])
...
for i in range(4):
    ...
    xopt = opt.minimize(LLF, np.array([np.mean(x)*blockLength,0]), args=(x, ARE, RMF, Energy, BACK, left, right),
                        bounds=([[1e-5, np.inf], [-50, 10]]))
    theta_hat = xopt['x']
    #theta_hat=np.array([np.mean(x)*blockLength,0])
    r = RMF_s_powerlaw(ARE, RMF, Energy, theta_hat, BACK, left, right)
    Cmin = 0.
    for j in range(blockLength):
        if x[j] == 0:
    ...
    #print(p_value_chi(Cmin, blockLength - 2),bootstrap_CAN(Cmin, theta_hat, blockLength, ARE,RMF,Energy,BACK,left,right,B),theory_test(Cmin, theta_hat,blockLength,X,I,ARE,RMF,Energy,BACK,left,right),bootstrap_test(Cmin, theta_hat, blockLength, ARE,RMF,Energy,BACK,left,right,B))
