...
def test(n,B,B1,B2,beta,iters,ARE,RMF,Energy,BACK=0.,left=0,right=0):
    ...
    for l in range(iters):
        ...
        if p_value_chi(Cmin, n - 2) < 0.05:
            reject1[l] = 1
        start = time.time()
        if bootstrap_CAN(Cmin, theta_hat, n, ARE,RMF,Energy,BACK,left,right,B) < 0.05:
            reject2[l] = 1
        end = time.time()
        ...
        if bootstrap_test(Cmin, theta_hat, n, ARE,RMF,Energy,BACK,left,right,B) < 0.05:
            reject4[l] = 1
        end = time.time()
        print('alg.4', end - start)
        #if bootstrap_bias(Cmin, theta_hat, n, B1, B2) < 0.05:
        #    reject5[l] = 1
        #start = time.time()
        #print('bias', start - end)
        #if double_boostrap(Cmin, theta_hat, n, B1, B2) < 0.05:
        #    reject6[l] = 1
        #end = time.time()
