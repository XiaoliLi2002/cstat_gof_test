...
def plot_covvsn_single(df,mu,alpha=0.10):
    ...
    if mu == 0.25:
        xtick_labels = ["1.75", "4.375", "8.75", "17.5", "35", "", "70"]
    elif mu == 2.5:
        xtick_labels = ["17.5", "43.75", "87.5", "175", "350", "", "700"]
    plt.xticks(n_params, xtick_labels)
    ref_line = np.linspace(7, 500, 100)
    plt.plot(ref_line, (alpha) * np.ones(len(ref_line)),
             'k--', alpha=0.5, linewidth=1,
             zorder=1)
    plt.title(f'Powerlaw, $K={mu},\Gamma=1, n\in[10,400]$')
...
if __name__=='__main__':
    ...
    plt.tight_layout()
    plt.savefig(f"figure/Figure11_Gamma=1.pdf", bbox_inches='tight')