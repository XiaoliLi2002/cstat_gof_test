...
from Plot_cov_width_n_Gamma1 import load_data_new
...
def plot_covvsmu_single(df,n,alpha=0.10):
    method_names = [r'Alg.1: LR-$\chi^2$ test', r'Alg.2c: Naive Z-test', r'Alg.3b: Corrected Z-test',
                    r'Alg.4: Param. Bootstrap']
    ...
    if n == 10:
        xtick_labels = ["0.7", "1.75", "3.5", "7", "11.2", "17.5", "35"]
    elif n == 50:
        xtick_labels = ["3.5", "8.75", "17.5", "35", "56", "87.5", "175"]
    plt.xticks(mu_params, xtick_labels)
    ref_line = np.linspace(0.07, 6, 100)
    plt.plot(ref_line, (alpha) * np.ones(len(ref_line)),
             'k--', alpha=0.5, linewidth=1,
             zorder=1)
    plt.title(f'Powerlaw, $n={n},\Gamma=1,K\in[0.1,5]$')
...
if __name__=='__main__':
    ...
    plt.tight_layout()
    plt.savefig(f"figure/Figure10_Gamma=1.pdf", bbox_inches='tight')