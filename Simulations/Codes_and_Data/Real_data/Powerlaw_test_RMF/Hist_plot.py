...
def test(n,B,B1,B2,beta,iters,ARE,RMF,Energy,BACK=0.,left=0,right=0):
    ...
    for l in range(iters):
        ...
        if m <= 20:
            ...
            maximum = 2 * m
        X = design_mat(ARE, RMF, Energy, beta, left, right)
        mean = expectation(s, n, X, I, maximum)
        std = math.sqrt(Var(s, n, X, I, maximum))
        ...
        x = np.arange(n - p - 3 * math.sqrt(2 * (n - p)), n - p + 3 * math.sqrt(2 * (n - p)), 0.05)
        ...
        x = np.arange(np.mean(C) - 3 * np.std(C), np.mean(C) + 3 * np.std(C), 0.05)
        y = normfun(x, np.mean(C), np.std(C))
        plt.plot(x, y, color='green', label='Alg.2b')

        x = np.arange(mean - 3 * std, mean + 3 * std, 0.05)
        y = normfun(x, mean, std)
        plt.plot(x, y, color='tomato', label='Alg.3b', linestyle='dashdot')
        ...
        #plt.title(r'$n=25,\frac{K}{n}=1,\alpha=1,R=D$', fontsize=18)
        plt.title(r'$n=25,\frac{K}{n}=1,\alpha=1,R=\frac{11^\top+nI}{2n}$', fontsize=18)
    ...
    for l in range(iters):
        ...
        plt.grid(linestyle='--', alpha=0.5)
        #plt.title(r'$n=25,\frac{K}{n}=2,\alpha=1,R=D$', fontsize=18)
        plt.title(r'$n=25,\frac{K}{n}=2,\alpha=1,R=\frac{11^\top+nI}{2n}$', fontsize=18)
    ...
    for l in range(iters):
        ...
        plt.grid(linestyle='--', alpha=0.5)
        #plt.title(r'$n=25,\frac{K}{n}=5,\alpha=1,R=D$', fontsize=18)
        plt.title(r'$n=25,\frac{K}{n}=5,\alpha=1,R=\frac{11^\top+nI}{2n}$', fontsize=18)

    plt.subplot(2,2,4)
    beta = np.array([10 * n, 1])
    p=len(beta)
    I=np.mat([1. for i in range(p)]).T
    ...
    for l in range(iters):
        ...
        plt.grid(linestyle='--', alpha=0.5)
        #plt.title(r'$n=25,\frac{K}{n}=10,\alpha=1,R=D$', fontsize=18)
        plt.title(r'$n=25,\frac{K}{n}=10,\alpha=1,R=\frac{11^\top+nI}{2n}$', fontsize=18)
    plt.tight_layout()
    plt.savefig('Figure,RMF2,n=25.pdf')
