...
import uncon_plugin,  con_theory, bootstrap_double, bootstrap_empirical
import bootstrap_empirical_parralled
import os
import time
...
def single_test_with_single_bootstrap_pvalue(n,beta,strue,snull,iters,B=1000,epsilon=1e-5,loc=0.5,strength=3,width=5):
    ...
    for i in range(int(iters)):  # repetition
        ...
        # start test
        time_now=time.time()
        pvalues_onesided[i,0], pvalues_twosided[i,0], _, _=Wilks_Chi2_test.p_value_chi(Cmin, n - len(betahat)) # Chisq test
        times[i,0]=time.time()-time_now

        time_now=time.time()
        pvalues_onesided[i,1], pvalues_twosided[i,1], _, _=uncon_plugin.uncon_plugin_test(Cmin, betahat, n, snull)# plugin uncon test
        times[i,1]=time.time()-time_now

        time_now=time.time()
        pvalues_onesided[i,2], pvalues_twosided[i,2], _, _= con_theory.con_theory_test(Cmin, betahat, n, snull) # Conditional test
        times[i,2]=time.time()-time_now

        time_now=time.time()
        pvalues_onesided[i,3], pvalues_twosided[i,3], _, _= bootstrap_empirical.bootstrap_test(Cmin,betahat,n,snull,B=B)  # Empirical Bootstrap test
        times[i,3]=time.time()-time_now

        time_now=time.time()
        pvalues_onesided[i,4], pvalues_twosided[i,4]= bootstrap_double.double_boostrap(Cmin,betahat,n,snull,B1=B,B2=B)  # Double Bootstrap test
...
if __name__=="__main__":
    ...
    save_dir = "results/datanewdouble/time/"
    ...
    # set dir
    save_dir = "results/datanewdouble/onesided"
    os.makedirs(save_dir, exist_ok=True)
    # toExcel
    filename=generate_filename_xlsx(params)
    ...
    save_dir = "results/datanewdouble/twosided"
    os.makedirs(save_dir, exist_ok=True)
    filename = generate_filename_xlsx(params)
    full_path = os.path.join(save_dir, filename)  # cat
    writer = pd.ExcelWriter(full_path, engine='xlsxwriter')
