...
def expectation(beta,n,X,I,max):
    s = generate_s_exp(n, beta[0], beta[1])
...
def bootstrap_test(Cmin,beta,n,B=1000):
    ...
    for i in range(B):
        ...
        theta1_hat = xopt['x'][0]
        theta2_hat = xopt['x'][1]
        r = generate_s_exp(n, theta1_hat, theta2_hat)
        for j in range(n):
            if x[j] == 0:
                C[i] +=r[j]
            else:
...
def bootstrap_bias(Cmin,beta,n,B1=1000,B2=1000):
    ...
    s=generate_s_exp(n,theta1_hat,theta2_hat)
    for i in range(B1):
        x = poisson_data(s)
        if x == [0 for i in range(n)]:
            continue
...
def LLF(theta,x):
    n=len(x)
    s=generate_s_exp(n,theta[0],theta[1])
    value=0
    for i in range(n):
...
test(n,B,B1,B2,[theta1,theta2],max,X,I,iters)