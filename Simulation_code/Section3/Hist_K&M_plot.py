...
for l in range(iters):
    ...
    #print(s)
    expec = expectation([theta1, theta2], n)
    ...
    for i in range(B):
        ...
        if x==[0 for y in range(n)]:
            continue
        xopt = opt.minimize(LLF, [theta1, theta2], bounds=([[1e-5, np.inf], [-10, 10]]))
        theta1_mle = xopt['x'][0]
        theta2_mle = xopt['x'][1]
        print(theta1_mle,theta2_mle)
    ...
    x = np.arange(Kaastra_mean(s) - 3 * math.sqrt(Kaastra_var(s)), Kaastra_mean(s) + 3 * math.sqrt(Kaastra_var(s)), 0.05)
    y = normfun(x, Kaastra_mean(s), math.sqrt(Kaastra_var(s)))
    plt.plot(x, y, color='blueviolet', label='Kaastra')
    ...
    plt.savefig('compare_n=10,mu=10.pdf')
